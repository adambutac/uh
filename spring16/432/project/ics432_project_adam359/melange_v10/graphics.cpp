...
bool sphere_ray_intersect(sphere *sph, vec &org, vec &dir, double *t, vec &pnt, vec &nrm){
  ...
  if (root > EPSILON) {
    ...
    if(tt <= EPSILON){
        ...
        vec_set(pnt, org.x+tt*dir.x, org.y+tt*dir.y, org.z+tt*dir.z);
        *t = tt;
        vec_set(nrm, (pnt.x-cnt->x)/sph->rad, (pnt.y-cnt->y)/sph->rad, (pnt.z-cnt->z)/sph->rad);
        return true;
    }
  } else {
    return false;
...
scene *scene_new(char *filename)
{
  ...
  scn->pixels = (double *)calloc(scn->window_w*scn->window_h*3, sizeof(double));
  return scn;
}
...
void *work(void *args){
  pack *pck = (pack *)args;
  scene *scn = pck->scn;
  pthread_mutex_t *mutex = pck->mutex;
  int x,y;
  while(y < scn->window_h){
    for(x = 0; x < scn->window_w; x++){
...
void scene_render(scene *scn)
{
  ...
  pthread_t *worker_t = (pthread_t*)calloc(NUM_THREADS, sizeof(pthread_t));
  ...
  for(int i = 0; i < NUM_THREADS; i++){
    pthread_create(worker_t+i, NULL, work, (void *)pck);
  }
                           ...
                           int generation, col &color)
{
    ...
    //FOR (i, num_illum_rays) {
      ...
      if (dot > 0) {
        ...
        if (trans > 0.0) {
          ...
          double sdot = vec_dot(refl, light_dir);
          if (sdot > 0) {
  ...
  if (mat->reflectivity > 0.0 && (generation < g_scene->max_ray_depth)) {
    //int num_refl_rays = 1;//material_num_refl_rays(mat);
    vec refl;
    vec_reflect(refl, dir, nrm);
...
void pixel_view_ray(double x, double y, vec *dir)
{
