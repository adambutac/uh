...
{
  double pixel_samples = scn->pixel_samples;
  double one_over_pixel_sample_sq = 1.0/(pixel_samples*pixel_samples);
  double dx, dy;
  int window_w,window_h,x,y,sx,sy;
  vec dir, camera_loc;
  col c;
  camera_loc = scn->camera_loc;
  window_w = scn->window_w;
  window_h = scn->window_h;
  float *p;

  for (y = 0; y < window_h; y++) {
    for (x = 0; x < window_w; x++) {
      for (sy = 0; sy < pixel_samples; sy++) {
        for (sx = 0; sx < pixel_samples; sx++) {
          dy = sy/pixel_samples;
          dx = sx/pixel_samples;
          pixel_view_ray(x+dx, y+dy, &dir);
          scene_ray_color(scn, camera_loc, dir, 0, c);
          //scene_add_pixel_color(scn, x, y, c, one_over_pixel_sample_sq);
          p = PIXEL(x, y);
          p[0] += (float)c.r * one_over_pixel_sample_sq;
          p[1] += (float)c.g * one_over_pixel_sample_sq;
          p[2] += (float)c.b * one_over_pixel_sample_sq;
