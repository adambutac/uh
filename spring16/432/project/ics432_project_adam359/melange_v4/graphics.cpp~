...
{
  double A,B,C,root,t1,t2,tt,dir_dot_nrm,rad;
  vec *cnt = &sph->cnt;
  vec org_minus_cnt;
  ...
  B = 2 * (dir.x*org_minus_cnt.x + dir.y*org_minus_cnt.y + dir.z*org_minus_cnt.z);
  C = (org_minus_cnt.x*org_minus_cnt.x +
       org_minus_cnt.y*org_minus_cnt.y +
       org_minus_cnt.z*org_minus_cnt.z) - rad*rad;
  ...
  if (root > EPSILON) {
    ...
    *t = tt;
    ...
    pnt = p;
    vec_set(nrm, (p.x-cnt->x)/sph->rad, (p.y-cnt->y)/rad, (p.z-cnt->z)/rad);
    dir_dot_nrm = dir.x*nrm.x + dir.y*nrm.y + dir.z*nrm.z;
