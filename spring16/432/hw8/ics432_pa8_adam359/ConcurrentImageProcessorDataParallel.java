...
public class ConcurrentImageProcessorDataParallel{
    ...
    public class ImageReader extends Thread{
        ...
        public ImageReader(ImageQueue q, ArrayList<File> f, String fil, int numProcs){
            filter = fil;
            qRef = q;
            fRef = f;
            numKills = numProcs;
        ...
        public void run(){
            ...
            /* Enqueue the poison pill. */
            for(int i = 0; i <  numKills; i++)
                qRef.enqueue(new Image(true));
        }
    }
    ...
    public class ImageProcessor extends Thread{
        ...
        public ImageProcessor(ImageQueue qIn, ImageQueue qOut, BufferedImageOp fil){
            qInRef = qIn;
            qOutRef = qOut;
            filter = fil;
        }
        ...
        public void run(){
            ...
            qOutRef.enqueue(new Image(true));
        }
    }

/* public class ConcurrentImageProcessorDataParallel{ */
    public static TimeRecordMap timer = new TimeRecordMap(); 
    private ImageQueue input, output;
    private ImageReader reader;
    private ImageWriter writer;
    private ImageProcessor[] proc;

    public ConcurrentImageProcessorDataParallel(ArrayList<File> files, 
                                            BufferedImageOp filter, String filterName){
        /* The queue that reader adds to, processor removes from. */
        input  = new ImageQueue(8);
        /* The queue that processor adds to, writer removes from. */
        output = new ImageQueue(8);
        ...
        reader = new ImageReader(input, files, filterName, 1); 
        writer = new ImageWriter(output, files);
        proc = new ImageProcessor[1];

        for(int i = 0; i < proc.length; i++)
           proc[i] = new ImageProcessor(input, output, filter);
    ...
    private static BufferedImageOp selectFilter(String select, int var){
        ...
        switch(select){
            ...
            case "smear":
                ...
                break;
            case "weird":
                filter = new DataParallelWeirdFilter(var);
                break;
            default:
                filter = null;
                break;
    ...
    public static void main(String args[]) {
        ...
        String usage = "Usage: <# threads> <filter name> <directory path>";
        ...
        int filterVar = Integer.parseInt(args[0]);

        /* Select the chosen filter. */ 
        String filterName = args[1];
        filter = selectFilter(filterName, filterVar);
        if(filter == null){
        ...
        files = getFilesInFolder(new File(args[2]));
        if(files == null){
            System.out.println(args[2] + " is not a directory.");
            System.exit(1);
        }
        
        ConcurrentImageProcessorDataParallel cip = new ConcurrentImageProcessorDataParallel(files, filter, filterName);
