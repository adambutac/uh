...
/* public class ConcurrentImageProcessorTaskParallel{ */
    ...
    public ConcurrentImageProcessorTaskParallel(ArrayList<File> files, String filter, int numProcs){
        ...
        ImageProcessor[] proc = new ImageProcessor[numProcs];

        reader.start();
        writer.start();
        for(ImageProcessor p: proc){
           p = new ImageProcessor(input, output, files, filter);
           p.start();
        } 
    ...
    public static void main(String args[]) {
        ...
        int numProcs = Integer.parseInt(args[0]);

        /* Select the chosen filter. */ 
        filter = selectFilter(args[1]);
        if(filter == null){
            System.out.println("Unknown filter \"" + args[1] + "\".");
            System.exit(1);
        }
        ...
        ConcurrentImageProcessorTaskParallel cip = new ConcurrentImageProcessorTaskParallel(files, args[1], numProcs);
